    ...
    if (submitted) {
      return (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          <div className="h-16 w-16 bg-gradient-to-br from-amber-500 to-amber-700 rounded-full flex items-center justify-center mb-4 shadow-[0_0_20px_rgba(245,158,11,0.5)]">
            <CheckCircle2 className="h-8 w-8 text-white" />
          </div>
          <h3 className="text-2xl font-bold text-amber-500 mb-2">Thank you for your valuable insight!</h3>
    ...
    return (
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Button 
          type="submit" 
          disabled={submitting}
          className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-6 text-lg shadow-[0_0_20px_rgba(245,158,11,0.4)] hover:shadow-[0_0_30px_rgba(245,158,11,0.6)] transition-all"
        >
      ...
      <DialogTrigger asChild>
        <button
          ...
          className="w-full md:w-auto px-6 py-3 text-lg font-extrabold rounded-lg 
                     bg-gray-800/80 text-amber-400 border-2 border-amber-600 
        ...
        >
          ...
          <span>PARTNER VALIDATION SURVEY (Enterprise Focus)</span>
        </button>
      </DialogTrigger>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto bg-card border-amber-600/50">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-amber-500" style={{ fontFamily: 'Orbitron, sans-serif' }}>
            Partner Solutions: Operational Validation
          </DialogTitle>
        </DialogHeader>
        <SurveyContent />
      </DialogContent>
