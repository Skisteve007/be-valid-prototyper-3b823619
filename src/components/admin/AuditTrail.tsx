...
import { auditApi } from '@/services/auditTrailApi';
import type { 
  EntryPointAuditLog, 
...
export const AuditTrail: React.FC<AuditTrailProps> = ({ className = '', onBack }) => {
  const rootClass = `min-h-screen bg-slate-950 pt-20 ${className}`.trim();
  ...
  if (loading && auditLogs.length === 0) {
    return (
      <div className={rootClass}>
        {/* Header */}
        <div className="border-b border-red-500/30 bg-gradient-to-r from-red-500/10 to-transparent backdrop-blur-xl">
