...
import { Shield, DollarSign, Users, Database, FileText, ChevronDown, ChevronRight } from 'lucide-react';
import { cn } from '@/lib/utils';
import { commandCenterApi, type AdminDashboard, type FeeConfigUpdate, type ScanFeeUpdate, type GhostPassPricingUpdate, type RetentionOverride, type PayoutAction } from '@/services/commandCenterApi';
...
export const CommandCenterTab: React.FC = () => {
  ...
  const loadDashboard = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await commandCenterApi.getDashboard();
      setDashboard(data);
  ...
  const handleUpdateFeeConfig = async () => {
    ...
    try {
      ...
      setError(null);
      await commandCenterApi.updateFeeConfig(feeConfig);
      await loadDashboard();
    } catch (err: any) {
  ...
  const handleUpdateScanFee = async () => {
    ...
    try {
      ...
      setError(null);
      await commandCenterApi.updateScanFee(scanFee);
      await loadDashboard();
      setScanFee({ venue_id: '', fee_cents: 10 });
    } catch (err: any) {
      setError(err.message || 'Failed to update scan fee');
    } finally {
      setUpdating(null);
    }
  };
  ...
  const handleUpdatePricing = async () => {
    try {
      setUpdating('pricing');
      setError(null);
      await commandCenterApi.updateGhostPassPricing(pricing);
      await loadDashboard();
    } catch (err: any) {
  ...
  const handlePayoutAction = async (payoutId: string, action: PayoutAction) => {
    try {
      ...
      setError(null);
      await commandCenterApi.processPayoutAction(payoutId, action);
      await loadDashboard();
    } catch (err: any) {
  ...
  const handleProcessAllPayouts = async () => {
    ...
    try {
      ...
      setError(null);
      await commandCenterApi.processAllPayouts();
      await loadDashboard();
    } catch (err: any) {
  ...
  const handleRetentionOverride = async () => {
    ...
    try {
      ...
      setError(null);
      await commandCenterApi.overrideRetention(retention);
      await loadDashboard();
      setRetention({ retention_days: 60, justification: '' });
    } catch (err: any) {
      setError(err.message || 'Failed to update retention period');
    } finally {
      setUpdating(null);
    }
  };
          ...
          >
            <div className="space-y-4 mt-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {[
                  { label: '1 Day Pass', key: 'one_day_cents' as const },
                  { label: '3 Day Pass', key: 'three_day_cents' as const },
                  { label: '5 Day Pass', key: 'five_day_cents' as const },
                  { label: '7 Day Pass', key: 'seven_day_cents' as const },
                  { label: '10 Day Pass', key: 'ten_day_cents' as const },
                  { label: '14 Day Pass', key: 'fourteen_day_cents' as const },
                  { label: '30 Day Pass', key: 'thirty_day_cents' as const }
                ].map(({ label, key }) => (
                  <div key={key}>
                    <label className="block text-sm font-medium text-slate-300 mb-2">
                      {label}
                    ...
                    <div className="relative">
                      ...
                      <input
                        type="number"
                        min="1"
                        max={key === 'thirty_day_cents' ? 150 : 100}
                        step="1"
                        value={pricing[key] / 100}
                        onChange={(e) => setPricing(prev => ({ ...prev, [key]: Math.round((parseFloat(e.target.value) || 0) * 100) }))}
